...
contextBridge.exposeInMainWorld('displayType', 'independent');
...
contextBridge.exposeInMainWorld('getOurNumber', async () => {
...
contextBridge.exposeInMainWorld(
  'sendDotMessage',
  (channelName, toPrivateUser, mdText) => {
...
ipcRenderer.on('get_page_description', (event, params) => {
  try {
    const metas = Array.from(document.getElementsByTagName('meta'));
    for (let i = 0; i < metas.length; i++) {
      const name = metas[i].getAttribute('name');
      if (name === 'description') {
        ipcRenderer.send('get_page_description_callback', {
          ...params,
    ...
    ipcRenderer.send('get_page_description_callback', params);
  } catch (e) {
    ipcRenderer.send('get_page_description_callback', params);
  }
});
