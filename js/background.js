...
(async function () {
  ...
  async function connect(firstRun) {
    ...
    if (firstRun === true) {
      // 修复第一次登录设置页面，主题样式radio没有选中的bug, 默认设置为light
    ...
    if (connectCount === 1) {
      ...
      window
        .getAccountManager()
        .setProfile({ meetingVersion, msgEncVersion: MSGENCVERSION });
      //上报meetingVersion
      //window.setMeetingVersion();
  ...
  async function mergeMembers(conversation, changedMembers, operator) {
    ...
    if (removedArray.length > 0) {
      ...
      if (removedArray.includes(ourNumber)) {
        await conversation.destroyMessages(); //被踢，客户端清空消息
        update.left = true;
  ...
  async function handleGroupChangeNotification(
    ...
    if (operator === ourNumber && operatorDeviceId.toString() === deviceId) {
      ...
      if (members && members.length > 0) {
        ...
        // 改变的群成员可能不是通讯录的人， 但是本地有跟他的会话并且 ext 有改动的话，这边刷一下该会话的 ext
    ...
    if (members && members.length > 0) {
      for (let i = 0; i < members.length; i++) {
        ...
        if (value && version) {
          ...
          // 本地没有 folder 或者通知过来的版本号更大
          if (!localFolder?.value || version >= localFolder?.version) {
            log.info('通知的 chat folder 数据', chatFolder);
            log.info('本地的 chat folder 数据', localFolder);

            // 对一些异常数据做一些过滤, 并对 type 和 folderType 做一个兼容
            const _value =
              value?.map(f => {
