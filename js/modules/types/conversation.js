...
async function maybeUpdateRemark(conversation, remark) {
  const { type } = conversation;
  if (type === 'group') {
    return null;
  }

  const { remarkHash = null, id } = conversation;

  let newHash = null;
  if (remark) {
    const data = new TextEncoder().encode(`${remark}-${id}`);
    newHash = await computeSha256(data);
  }

  if (remarkHash !== newHash) {
    return { remarkHash: newHash };
  }
