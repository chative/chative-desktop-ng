  ...
  async decrypt(envelope, ciphertext) {
    ...
    switch (envelope.type) {
      ...
      case textsecure.protobuf.Envelope.Type.ENCRYPTEDTEXT:
        ...
        if (
          oriVersion > window.MESSAGE_CURRENT_VERSION ||
          oriVersion < window.MESSAGE_MINIMUM_SUPPORTED_VERSION
        ) {
          window.log.info(
            'ENCRYPTEDTEXT message bad version:' + oriVersion,
            this.getEnvelopeId(envelope)
          );
          promise = Promise.reject(
            new Error('ENCRYPTEDTEXT message bad version:' + oriVersion)
          );
          break;
        }
        ...
        * */
        ...
        if (!decResult.verified_id_result) {
          ...
          // promise = Promise.reject(
          //   new Error('ENCRYPTEDTEXT message decrypt_message failed!!!')
          // );
          // break;
        }
    ...
    if (content.syncMessage) {
      ...
      if (
        ...
        envelope.timestamp.toNumber() !==
          content.dataMessage.timestamp.toNumber()
      ) {
        errorMessage =
          'Ignoring message with non-matching data message timestamp';
        console.log(errorMessage + envelope.source);
      } else {
    ...
    this.reportException(
      {
        ...
        message: errorMessage || 'Unsupported content message',
      },
      envelope
    );
