...
(function () {
  ...
  window.WebSocketResource = function WebSocketResource(socket, opts = {}) {
    ...
    this.checkStatus = () => {
      window.log.info('checking websocket status ...');

      if (this.keepalive) {
        this.keepalive.reset();
        this.keepalive.check();
      }
    };
  ...
  KeepAlive.prototype = {
    ...
    check() {
      ...
      window.log.info('Sent a keepalive message success.');
    },
    reset() {
      clearTimeout(this.keepAliveTimer);
      clearTimeout(this.disconnectTimer);
      this.keepAliveTimer = setTimeout(() => this.check(), 55000);
    },
  };
})();
