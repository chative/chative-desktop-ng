...
export class LeftPane extends React.Component<Props, State> {
    ...
    try {
      ...
      if (operationType === 'delete') {
        ...
        if (operationType === 'create') {
          ...
          if (boxScroll) {
            if (isRecommended) {
              // 如果是添加推荐的，添加到头部, 滚动条设置到最左边
              setTimeout(() => {
                boxScroll.scrollTo({ left: 0, behavior: 'smooth' });
              });
            } else {
              // 如果是手动创建，直接追加到 chat folder 的尾巴， 所以直接滑动到最后一个
              // 滚动条设置到最右边
              setTimeout(() => {
                // 这边设置延时，等页面渲染完毕后再操作，否则无法滑动到最新的那一个，因为数据没更新使用的是旧数据。暂且这样做。
                // 设置的 left ： 100000, 直接设置一个溢出的，就会滑动到最新的。
  ...
  public handleClickToEdiChatFolder = async (
    ...
    try {
      ...
      if (isBarOperation) {
        ...
        // 这边设置一下延时，因为 showChatFolder 渲染 chat folder 后 chatFolderRef 才 不为 null。
        setTimeout(() => {
  ...
  public checkConversationToFolder = (folderName: string) => {
    ...
    // 如何当前选择的会话存在于某个分组。
    // 1.转移的分组没有当前会话，直接push。
    // 2.当前转移的分组已经有了当前会话， 从转移的分组中去除改会话。如果当前分组只有当前这一个会话了，不予操作。
  ...
  public chatFolderNoticeChange = (event: any) => {
    ...
    // 判断当前传过来的数据中，folders中是否包含当前所在分组，如果包含就不需要跳转分组了，如果不包含说明当前这个分组被删除了。就跳转到Total。
  ...
  public formatFolder(folders: any) {
    return (
      folders?.map((f: any) => {
        ...
        // 因为一些中间版本有问题，暂时先刷一下推荐 folder 的type，迭代几个版本后在看下去除掉
          ...
          <div
            ...
            onMouseDown={event => {
              if (event && event.button === 0) {
                let type = undefined;
                if (currentFolder !== 'all_conversation_unique') {
                  // 默认(undefined)删除all_conversation_unique里面的会话, 1-从folder中移除会话, 2-移除通过条件筛选的会话
                  const folder =
