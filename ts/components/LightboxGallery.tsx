...
export class LightboxGallery extends React.Component<Props, State> {
  ...
  public render() {
    const { close, media, onSave, i18n, onCopyImage } = this.props;
    ...
    const copyImageCallback = onCopyImage ? this.handleCopyImage : undefined;
  ...
  private readonly handleCopyImage = () => {
    const { media, onCopyImage } = this.props;
    if (!onCopyImage) {
      return;
    }
