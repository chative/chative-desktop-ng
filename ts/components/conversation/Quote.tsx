...
export class Quote extends React.Component<Props, State> {
  ...
  public renderGenericFile() {
    const { attachment, isIncoming, isReply, text, messageMode, i18n } =
      this.props;
    ...
    return (
      <div className="module-quote__generic-file">
        {isReply ? (
          <div>
            <span className={'module-reply-span'}>Form:</span>
          </div>
        ) : null}
        {!messageMode && <div className="module-quote__generic-file__icon" />}
        ...
        >
          {messageMode
            ? '[' + i18n('confidential-message-description') + ']'
            : fileName}
        </div>
      </div>
    );
  ...
  public renderText() {
    const { i18n, text, attachment, isIncoming, isReply, messageMode } =
      this.props;
    ...
    if (text) {
      return (
        ...
        >
          {isReply ? (
            <div>
              <span className={'module-reply-span'}>From:</span>
            </div>
          ) : null}
          {messageMode ? (
            '[' + i18n('confidential-message-description') + ']'
    ...
    if (typeLabel) {
      return (
        ...
        >
          ...
          {messageMode
            ? '[' + i18n('confidential-message-description') + ']'
            : typeLabel === i18n('photo')
            ? i18n('shortForAttachment')
            : typeLabel}
        </div>
      );
    }
  ...
  public renderAuthor() {
    ...
    return (
      ...
      >
        {isReply ? (
          <div>
            <span className={'module-reply-span'}>Topic</span>
          </div>
        ) : null}
        {isFromMe ? (
          i18n('you')
