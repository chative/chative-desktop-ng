...
const withSection =
  ...
  (mediaItem: MediaItemType): MediaItemWithSection => {
    ...
    const { message } = mediaItem;
    const mediaItemServerDate = moment.utc(message.serverTimestamp);
    if (mediaItemServerDate.isAfter(today)) {
      return {
        order: 0,
        type: 'today',
        mediaItem,
    ...
    if (mediaItemServerDate.isAfter(yesterday)) {
      return {
        order: 1,
        type: 'yesterday',
        mediaItem,
    ...
    if (mediaItemServerDate.isAfter(thisWeek)) {
      return {
        order: 2,
        type: 'thisWeek',
        mediaItem,
    ...
    if (mediaItemServerDate.isAfter(thisMonth)) {
      return {
        order: 3,
        type: 'thisMonth',
        mediaItem,
    ...
    const month: number = mediaItemServerDate.month();
    const year: number = mediaItemServerDate.year();
