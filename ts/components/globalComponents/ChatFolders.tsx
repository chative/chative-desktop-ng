...
export default function ChatFolders(props: Props) {
  ...
  //初始化folder 的顺序，用于后面对比，顺序一样就不走网络操作了。 初始化 folder conditions
  useEffect(() => {
  ...
  function handleCreateOrUpdate() {
    ...
    if (RECOMMEND_FOLDER_NAMES.includes(inputFolderName)) {
      if (operationType === 'create') {
        ...
        // 当前是编辑folder的状态，推荐folder不可以被编辑的
        // 所以如果有跟推荐folder名字重合，只可能是之前就有的folder那么，然后如果编辑过程中没有改变名字，还是让他保存。
        // 如果改变了还是跟推荐folder名字有重合，那就不让他保存。
        if (
    ...
    let cIds = [] as any; // 已选择的会话的 id ，因为是要保存到服务端，所以群会话的 id 要转换成 idv2 进行保存。
    ...
    if (operationType === 'edit') {
      ...
      // 如果在进行编辑操作时，还未保存，但是手机端删除了正在编辑的分组。
      // 所以虽然处于编辑状态，但是数据库中已经没有该分组。 这边进行保存的时候，转换为新增操作。
      if (flag) {
        targetChatFolders.push(folder);
      }
    }
