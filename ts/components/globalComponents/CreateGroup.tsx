...
export default function CreateGroup(props: PropsType) {
  ...
  const renderByLeader = async (filterLeader: Array<any>) => {
    try {
      ...
      for (let i = 0; i < filterLeader.length; i += 1) {
        // 看看当前这个 leader 下的人的列表缓存是否有， 有的话直接用，没有的话拉取一下接口
  ...
  const switchCondition = (condition: string) => {
    ...
    // 选择的和当前的不一样，更新 preCondition, 将焦点聚到输入框并显示可选择的列表，更新用户列表
  ...
  const handleClick = (event: MouseEvent) => {
    if (event) {
      // 点击条件选择，其他的列表都应该消失，只控制条件列表的显示和隐藏。
      if (
      ...
      // 如果是点击选择条件，这边直接返回，不做处理。 相应的处理再点击条件时做。 这边不用 ref 的方式，因为点击和
      if (
