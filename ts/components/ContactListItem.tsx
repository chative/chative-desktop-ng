...
export class ContactListItem extends React.Component<Props> {
  ...
  public render() {
    ...
    /**
     * 联系人匹配中
    - ID匹配灰字顺序：签名 > ID > email > BU > Dept > Leader  >空
    - BU匹配灰字顺序：签名 > BU > email > Dept > Leader > ID >空
    - 其它匹配：签名 > email > BU > Dept > Leader > ID >空

    是有可能出现只有名字、ID两行的情况的，所以如果只有这两个信息，就显示两行，其它情况都尽量显示三行～
