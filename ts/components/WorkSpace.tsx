...
export class WorkSpace extends React.Component<PropsType, StateType> {
  ...
  constructor(props: any) {
    ...
    if ((window as any).getMiniProgramList) {
      ...
      for (let i = 0; i < miniProgramList.length; i += 1) {
        // type 为 -1 的 app， 表示需要隐藏.
        if (miniProgramList[i]?.type === -1) {
          continue;
        }

        // 0x1: 仅macOS ,0x2: 仅iOS , 0xFF: 所有
      ...
      <>
        <AppDetailDialog
          ...
          openApp={() => {
            ...
            const type = currentAppItem.type; // 0-独立窗口打开，1-外部浏览器打开，2-侧边窗口打开
            // 打开浏览器
            if (type === 1) {
  ...
  private sortAgainByPin = (programList?: any, search?: string) => {
    ...
    unpinArr = deduplicationArr.slice(arr.length, deduplicationArr.length + 1); //没有pin的app
    const arrLast = this.sortByVpn(unpinArr);
  ...
  private updateMiniProgramList = (ev: any) => {
    if (!ev || !ev.detail) {
      this.setState({ loadingState: 1 }); // 获取失败了
      return;
    }
    ...
    for (let i = 0; i < list.length; i += 1) {
      ...
      theList.push(
        <div
          ...
          onMouseDown={event => {
            if (event?.button !== 2) {
              const type = item.type; // 0-独立窗口打开，1-外部浏览器打开，2-侧边窗口打开
              // 打开浏览器
              if (type === 1) {
