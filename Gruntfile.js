...
module.exports = grunt => {
  ...
  const updateLocalJson = (jsonData, renew) => {
    const jsonFile = 'config/local-production.json';

    const productionJson = {};

    // should renew local json file when renew is true
    if (renew || !grunt.file.exists(jsonFile)) {
      Object.assign(productionJson, jsonData);
    } else {
      Object.assign(productionJson, grunt.file.readJSON(jsonFile), jsonData);
    }

    grunt.file.write(jsonFile, `${JSON.stringify(productionJson, null, 2)}`);
  };
  ...
  grunt.registerTask('getExpireTime', () => {
    ...
    const renew = true;
    updateLocalJson({ buildExpiration: time }, renew);
  });

  grunt.registerTask('getCommitInfo', () => {
    grunt.task.requires('gitinfo');
    const gitinfo = grunt.config.get('gitinfo');
    const lastCommitTime = gitinfo.local.branch.current.lastCommitTime;
    const lastCommitSha = gitinfo.local.branch.current.SHA;

    updateLocalJson({ lastCommitSha, lastCommitTime });
  });

  grunt.registerTask('getBuildInfo', () => {
    const buildInfo = {};

    const CIBuildNumber = process?.env?.CI_BUILD_NUMBER;
    if (CIBuildNumber) {
      buildInfo.CIBuildNumber = CIBuildNumber;
    }

    buildInfo.buildAt = new Date().toISOString();

    updateLocalJson(buildInfo);
  });

  grunt.registerTask('getBuildVersion', () => {
    const version = semver.parse(packageJson.version);
    version.patch = `${moment().format('YYMMDD')}00`;

    packageJson.version = version.format();
    packageJson.build.mac.target = [{ target: 'dmg', arch: ['x64', 'arm64'] }];
    packageJson.build.mac.requirements = './build/requirements.txt';
